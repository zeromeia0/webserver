...
ROUTE Server::findRoute(std::string path) {
	ROUTE ret;
	size_t longest = 0;
	for (std::vector<ROUTE>::const_iterator it = S_CONFIG->router.begin(); it != S_CONFIG->router.end(); ++it ) {
...
void Server::OUT() {
	...
	ROUTE		ROUT = findRoute(RES->headers.path);
	METHOD		METH = RES->headers.method;
	std::string	PATH = ROUT.root + RES->headers.path;
	RES->payload.clear();
	RES->payloadLen = 0;

	bool MethodNotAllowed = !valueInContainer(getMethodTxt(METH), ROUT.methods);
	bool PayloadTooLarge = curClient->REQ->payloadLen > S_CONFIG->clientMaxBodySize;
	...
	if (MethodNotAllowed) {
		...
		if (!(access(RES->headers.path.c_str(), F_OK) == 0)) {
			...
			std::string *ret = cgi((const char *)args[0], args, envp);
			if (ret) {
				RES->addPayload(*ret);
				RES->status_code = 200;
			} else {
		...
		switch (METH) {
			...
			case POST: {
				RES->headers.path = PATH;
				writeFileContent(RES->headers.path, RES->payload);
				ERROR(201);
				break;
			}
			case DELETE: {
